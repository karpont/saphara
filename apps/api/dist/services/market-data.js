...
export async function getPartMarketData() {
    return cached("part_market", async () => {
        // 1) GeckoTerminal — en detaylı on-chain veri
        try {
            const res = await fetch(`https://api.geckoterminal.com/api/v2/networks/bsc/tokens/${PART_CONTRACT}`, { headers: { "Accept": "application/json;version=20230302" } });
            if (res.ok) {
        ...
        try {
            const res = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${PART_CONTRACT}`, { headers: { "User-Agent": "Saphara/1.0" } });
            if (res.ok) {
                const data = await res.json();
...
export async function getBnbData() {
    return cached("bnb_price", async () => {
        try {
            const res = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd&include_24hr_vol=true&include_24hr_change=true&include_market_cap=true", { headers: { "Accept": "application/json" } });
            if (!res.ok)
...
export async function getTopCrypto() {
    return cached("top_crypto", async () => {
        try {
            const res = await fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false", { headers: { "Accept": "application/json" } });
            if (!res.ok)
                throw new Error("CoinGecko error");
            const data = (await res.json());
            return data.map((c) => ({
...
export async function getPartPriceHistory() {
    return cached("part_history", async () => {
        try {
            const res = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${PART_CONTRACT}`, { headers: { "User-Agent": "Saphara/1.0" } });
            if (!res.ok)
