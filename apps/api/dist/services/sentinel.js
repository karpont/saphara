...
const errorRing = [];
const securityRing = [];
const requestCounts = new Map(); // ip → istek sayısı (son dakika)
export function recordError(level, msg, ctx) {
    errorRing.push({ at: Date.now(), level, msg, ctx });
    if (errorRing.length > 200)
        errorRing.shift();
}
export function recordSecurityEvent(type, detail, ip) {
    securityRing.push({ at: Date.now(), type, ip, detail });
    if (securityRing.length > 100)
        securityRing.shift();
}
...
setInterval(() => requestCounts.clear(), 60000).unref();
// ── Sağlık kontrolleri ────────────────────────────────────────────────────
async function runHealthChecks() {
    ...
    try {
        ...
        checks.push({ name: "Veritabanı", ok: false, detail: e.message });
    }
    // Haber API anahtarı
    checks.push({
        name: "Haber kaynağı",
...
async function notifyOwnerRealtime(count, criticals) {
    if (Date.now() - lastAlertAt < 300000)
        return;
