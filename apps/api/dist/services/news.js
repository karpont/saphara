...
async function fetchCryptoPanic(filter = "hot") {
...
async function fetchCryptoRss() {
    const results = await Promise.allSettled(CRYPTO_RSS_FEEDS.map(async ({ url, source }) => {
...
function inferSentimentFromTitle(title) {
...
function calcSentimentScore(items) {
    ...
    for (const it of items) {
        ...
        if (it.votes)
            score += (it.votes.positive - it.votes.negative) * 0.1;
    }
...
function getFearGreed(score) {
    ...
    if (clamped >= 80)
        return { label: "Aşırı Açgözlülük", value: clamped };
    if (clamped >= 60)
        return { label: "Açgözlülük", value: clamped };
    if (clamped >= 45)
        return { label: "Nötr", value: clamped };
    if (clamped >= 25)
        return { label: "Korku", value: clamped };
    return { label: "Aşırı Korku", value: clamped };
}
function buildMarketNote(score, label) {
    if (score > 30)
        return `Piyasa genel olarak yükseliş eğiliminde görünüyor. ${label} seviyesinde işlem yapılıyor. Dikkatli olun — aşırı iyimserlik düzeltme riskini artırabilir.`;
    if (score > 5)
        return `Hafif yükseliş haberleri öne çıkıyor. Piyasa temkinli iyimser bir tutum sergiliyor.`;
    if (score < -30)
        return `Olumsuz haberler baskın. ${label} seviyesindeki korku genellikle iyi bir satın alma fırsatı olarak değerlendirilir, ancak risk yönetimine dikkat edin.`;
    if (score < -5)
        return `Hafif düşüş yönlü haberler mevcut. Piyasa temkinli bir seyir izliyor.`;
...
export async function getMarketAnalysis() {
    ...
    if (hit && Date.now() - hit.at < ANALYSIS_TTL)
        return hit.data;
    ...
    const bullItems = cpBull.status === "fulfilled" ? (cpBull.value ?? []).map(i => ({ ...i, sentiment: "bullish" })) : [];
    const bearItems = cpBear.status === "fulfilled" ? (cpBear.value ?? []).map(i => ({ ...i, sentiment: "bearish" })) : [];
...
async function fetchGNews(category, lang) {
    if (!GNEWS_KEY)
        return [];
    try {
...
async function fetchNewsData(category, lang) {
    if (!NEWSDATA_KEY)
        return [];
    try {
...
async function fetchHackerNews() {
    try {
        ...
        const stories = await Promise.allSettled(ids.slice(0, 12).map((id) => fetch(`https://hacker-news.firebaseio.com/v0/item/${id}.json`, { signal: AbortSignal.timeout(5000) }).then((r) => r.json())));
        return stories
...
async function fetchSportsNews() {
    try {
        ...
        if (!res.ok)
            return [];
        const json = await res.json();
        return (json.events ?? []).slice(0, 10).map((e, i) => ({
            id: `sports_${i}`,
...
/* ── Ana getirici ─────────────────────────────────────────────── */
export async function getNews(category = "general", lang = "tr") {
    const cacheKey = `${category}:${lang}`;
    const hit = cache.get(cacheKey);
    if (hit && Date.now() - hit.at < TTL_MS)
        return hit.data;
    if (!apiLimiter.allow("news"))
        return hit?.data ?? [];
    let items;
    switch (category) {
        case "crypto": {
