...
async function createBotPost(botId) {
    ...
    if (strategy < 0.3) {
        ...
        if (Math.random() > 0.6)
            mediaUrl = imageUrl(`price_${Date.now()}`);
    }
        ...
        text = pickRandom(TEMPLATES.saphara_promo)
            ...
            .replace("{POSTS}", String(Math.floor(Math.random() * 50 + 10)));
        if (Math.random() > 0.4)
            mediaUrl = imageUrl(`promo_${Date.now()}`);
    }
    else {
        // %25: lifestyle + kripto
        text = pickRandom(TEMPLATES.lifestyle);
        if (Math.random() > 0.5)
            mediaUrl = imageUrl(`life_${Date.now()}`);
    }
...
export async function runBotCycleOnce(count = 5) {
    const bots = await prisma.user.findMany({
