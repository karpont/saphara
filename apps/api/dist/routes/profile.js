...
export async function registerProfileRoutes(app) {
    ...
    app.get("/users/:handle", async (req, reply) => {
        ...
        if (!user)
            return reply.code(404).send({ error: "Kullanici bulunamadi" });
        return user;
    });
    // Takip et
    app.post("/follow/:id", async (req, reply) => {
        ...
        const { id } = req.params;
        if (id === followerId)
            return reply.code(400).send({ error: "Kendini takip edemezsin" });
    ...
    app.patch("/me", async (req, reply) => {
        ...
        if (coverUrl !== undefined)
            data.coverUrl = validateImageUrl(coverUrl);
        if (avatarUrl !== undefined)
            data.avatarUrl = validateImageUrl(avatarUrl);
