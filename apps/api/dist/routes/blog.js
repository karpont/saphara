...
const VALID_CATEGORIES = ["general", "crypto", "defi", "nft", "tutorial", "analysis", "community", "technology", "market", "lifestyle"];
/* Paragraph.xyz / Mirror.xyz modelinden ilham */
const BLOG_CREATION_FEE_PART = 50; // her yeni blog yazısı için 50 PART
const MAX_CONTENT_LENGTH = 50000; // ~10k kelime sınırı
...
function estimateReadingMins(content) {
...
export async function registerBlogRoutes(app) {
    ...
    app.get("/blog", async (req) => {
        const { category, featured, limit = "20", cursor } = req.query;
    ...
    app.post("/blog", async (req, reply) => {
        ...
        if (!userId)
            return;
        const { title, content, excerpt, coverUrl, category, tags, hasPoll, pollQuestion, pollOptions, featured, } = req.body;
        /* ── Temel validasyon ── */
