...
export async function registerDaoRoutes(app) {
    /* ── List proposals ────────────────────────────────────────── */
    ...
    app.post("/dao/proposals", async (req, reply) => {
        ...
        if (!userId)
            return;
        const { title, description, type = "general", endsAt, treasuryAmount, treasuryRecipient, tags = [], } = req.body;
        if (!title?.trim())
            return reply.code(400).send({ error: "Başlık gerekli" });
        if (!description?.trim())
            return reply.code(400).send({ error: "Açıklama gerekli" });
        if (!endsAt)
            return reply.code(400).send({ error: "Bitiş tarihi gerekli" });
        const endsAtDate = new Date(endsAt);
        if (endsAtDate <= new Date())
            return reply.code(400).send({ error: "Bitiş tarihi gelecekte olmalı" });
    ...
    app.post("/dao/proposals/:id/vote", async (req, reply) => {
        ...
        if (!proposal)
            return reply.code(404).send({ error: "Teklif bulunamadı" });
        if (proposal.status !== "active")
            return reply.code(400).send({ error: "Oylama aktif değil" });
        if (new Date() > proposal.endsAt)
            return reply.code(400).send({ error: "Oylama süresi doldu" });
        const existing = await prisma.daoVote.findUnique({
            where: { proposalId_voterId: { proposalId: id, voterId: userId } },
        });
        if (existing)
            return reply.code(409).send({ error: "Zaten oy kullandınız" });
