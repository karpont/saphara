...
function generateAttributes(rarity) {
...
export async function registerNftRoutes(app) {
    ...
    app.get("/nft/collections/:id/tokens", async (req) => {
        const { id } = req.params;
        const { limit = "24", offset = "0", listed } = req.query;
    ...
    app.post("/nft/collections/:id/whitelist/apply", async (req, reply) => {
        ...
        if (existing)
            return reply.code(409).send({ error: "Already on whitelist", entry: existing });
    ...
    app.post("/nft/collections/:id/mint", async (req, reply) => {
        ...
        if (!userId)
            return;
        const { id } = req.params;
        const { quantity = 1, useWhitelist = false } = req.body;
        if (quantity < 1 || quantity > 5)
            return reply.code(400).send({ error: "Mint miktarı 1-5 arasında olmalı" });
        const col = await prisma.nftCollection.findUnique({ where: { id } });
        if (!col)
            return reply.code(404).send({ error: "Collection not found" });
        if (col.status !== "active")
            return reply.code(400).send({ error: "Mint şu an aktif değil" });
        ...
        const tokens = await prisma.$transaction(Array.from({ length: quantity }, (_, i) => {
    ...
    app.post("/nft/tokens/:tokenId/list", async (req, reply) => {
        ...
        if (!token)
            return reply.code(404).send({ error: "Token bulunamadı" });
    ...
    app.get("/nft/marketplace", async (req) => {
        const { limit = "24", rarity, collection } = req.query;
        const tokens = await prisma.nftToken.findMany({
            where: {
                listed: true,
                ...(rarity ? { rarity } : {}),
    ...
    app.post("/nft/tokens/:tokenId/buy", async (req, reply) => {
        ...
        if (!token)
            return reply.code(404).send({ error: "Token bulunamadı" });
        if (!token.listed)
            return reply.code(400).send({ error: "Bu NFT satışta değil" });
        if (token.ownerId === userId)
            return reply.code(400).send({ error: "Kendi NFT'nizi satın alamazsınız" });
        const price = Number(token.listingPrice ?? 0);
        /* ERC-2981: creator royalty — collection.royaltyPct stored as % (e.g. 5 = 5%) */
        const rawRoyaltyPct = Number(token.collection?.royaltyPct ?? 0) / 100;
        ...
        const creatorId = token.collection?.creatorId;
        await prisma.$transaction([
