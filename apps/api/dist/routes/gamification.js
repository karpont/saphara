...
const LEVEL_XP = (lvl) => Math.floor(100 * Math.pow(lvl, 1.6));
const xpToLevel = (xp) => {
    let lvl = 1;
    while (LEVEL_XP(lvl + 1) <= xp)
        lvl++;
    return lvl;
};
const LEVEL_TITLES = {
...
export async function registerGamificationRoutes(app) {
    ...
    app.post("/gamification/quests/:id/progress", async (req, reply) => {
        ...
        if (!quest || !quest.active)
            return reply.code(404).send({ error: "Görev bulunamadı" });
    ...
    app.post("/gamification/quests/:id/claim", async (req, reply) => {
        ...
        if (!progress?.completed)
            return reply.code(400).send({ error: "Görev tamamlanmadı" });
        if (progress.claimedAt)
            return reply.code(409).send({ error: "Ödül zaten alındı" });
    ...
    app.post("/gamification/referral/use", async (req, reply) => {
        ...
        if (!referrerStats)
            return reply.code(404).send({ error: "Geçersiz referral kodu" });
        if (referrerStats.userId === userId)
            return reply.code(400).send({ error: "Kendi kodunuzu kullanamazsınız" });
        const myStats = await getOrCreateStats(userId);
        if (myStats.referredBy)
            return reply.code(409).send({ error: "Referral kodu zaten kullandınız" });
