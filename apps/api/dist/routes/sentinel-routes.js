...
async function isOwner(req, reply) {
    const userId = requireAuth(req, reply);
    if (!userId)
        return false;
    if (!TREASURY) {
        reply.code(403).send({ error: "Treasury tanımlı değil" });
        return false;
    }
...
export function isIPBlocked(ip) { return blockedIPs.has(ip); }
export async function registerSentinelRoutes(app) {
    /* ── Son raporu getir (önbellekten) ───────────────────────────────── */
    app.get("/admin/sentinel/report", async (req, reply) => {
        if (!await isOwner(req, reply))
            return;
    ...
    app.post("/admin/sentinel/action", async (req, reply) => {
        ...
        if (!action || !target)
            return reply.code(400).send({ error: "action ve target zorunlu" });
        switch (action) {
            case "block_ip": {
    ...
    app.get("/admin/sentinel/stats", async (req, reply) => {
        if (!await isOwner(req, reply))
            return;
        const [totalUsers, totalPosts, totalReels, totalCampaigns, activeCampaigns, totalTips, totalMessages,] = await Promise.all([
            prisma.user.count(),
            prisma.post.count(),
            prisma.reel.count(),
