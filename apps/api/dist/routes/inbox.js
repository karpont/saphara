...
export async function registerInboxRoutes(app) {
    // --- Bildirimler ---
    ...
    app.post("/messages", async (req, reply) => {
        ...
        if (to === me)
            return reply.code(400).send({ error: "Kendinize mesaj gönderemezsiniz" });
        if (text?.trim()) {
    ...
    app.get("/conversations", async (req, reply) => {
        ...
        for (const m of recent) {
            ...
            if (!seenSet.has(other)) {
                seenSet.add(other);
                orderedOthers.push(other);
            }
            if (!lastMsgMap.has(other))
                lastMsgMap.set(other, m);
        }
...
export async function notify(userId, kind, text) {
    await prisma.notification.create({ data: { userId, kind: kind, text } });
