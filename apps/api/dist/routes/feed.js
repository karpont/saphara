import { rankFeed } from "@saphara/recommendation";
...
export async function registerFeedRoutes(app) {
    // Akis: son gonderileri cek, oneri paketiyle sirala
    app.get("/feed", async (req) => {
        const { cursor, limit, scope } = req.query;
    ...
    app.post("/posts", async (req, reply) => {
        ...
        if (!cleanText && !mediaUrl && !(mediaUrls?.length))
            return reply.code(400).send({ error: "Boş gönderi" });
        if (cleanText) {
            const check = moderate(cleanText);
            if (!check.ok)
                return reply.code(422).send({ error: check.message, reason: check.reason });
        }
    ...
    app.post("/reels", async (req, reply) => {
        ...
        if (!videoUrl)
            return reply.code(400).send({ error: "videoUrl gerekli" });
