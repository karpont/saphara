...
export async function registerEventRoutes(app) {
    ...
    app.post("/events", async (req, reply) => {
        ...
        if (!userId)
            return;
        const { title, description, imageUrl, startAt, endAt, location, online, maxAttendees, pricePart } = req.body;
        if (!title || !startAt)
            return reply.code(400).send({ error: "Başlık ve başlangıç tarihi gerekli" });
    ...
    app.post("/events/:id/attend", async (req, reply) => {
        ...
        if (!event)
            return reply.code(404).send({ error: "Etkinlik bulunamadı" });
