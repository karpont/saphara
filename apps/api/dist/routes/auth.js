...
function issueTokenPair(userId, address) {
...
export async function registerAuthRoutes(app) {
    ...
    app.post("/auth/verify", async (req, reply) => {
        ...
        if (!message || !signature)
            return reply.code(400).send({ error: "message ve signature gerekli" });
        let fields;
        try {
            fields = parseSiweMessage(message);
        }
        catch {
            return reply.code(400).send({ error: "Gecersiz SIWE mesaji" });
        }
        // Nonce kontrolu (tek kullanimlik + sureli)
        const exp = fields.nonce ? nonces.get(fields.nonce) : undefined;
        if (!exp || exp < Date.now())
            return reply.code(401).send({ error: "Nonce gecersiz veya suresi dolmus" });
        ...
        if (!valid)
            return reply.code(401).send({ error: "Imza dogrulanamadi" });
