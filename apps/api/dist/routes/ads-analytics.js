...
const PRICING_MODEL = {
    cpm: {
        ...
        tiers: [
            { label: "Standart", cpmUsdt: 3, cpmPart: 2.4, minImpressions: 10000 },
            { label: "Premium", cpmUsdt: 6, cpmPart: 4.8, minImpressions: 50000 },
            { label: "Spotlight", cpmUsdt: 12, cpmPart: 9.6, minImpressions: 200000 },
        ],
    },
    cpc: {
    ...
    durationPackages: [
        ...
        { days: 7, label: "Haftalık", budgetMinUsdt: 100, bonusImpressions: 5000, popular: true },
        { days: 30, label: "Aylık", budgetMinUsdt: 300, bonusImpressions: 25000, popular: false },
        { days: 90, label: "3 Aylık", budgetMinUsdt: 750, bonusImpressions: 80000, popular: false },
    ],
...
export async function registerAdAnalyticsRoutes(app) {
    ...
    app.post("/ads/campaigns", async (req, reply) => {
        ...
        const b = req.body;
        if (!b.name?.trim())
            return reply.code(400).send({ error: "Kampanya adı zorunlu" });
        if (!b.objective)
            return reply.code(400).send({ error: "Hedef zorunlu" });
        if (!VALID_OBJ.includes(b.objective))
            return reply.code(400).send({ error: "Geçersiz hedef" });
        if (!b.budgetPart || !b.bidPart)
            return reply.code(400).send({ error: "Bütçe ve teklif zorunlu" });
        ...
        for (const field of [b.name, b.headline, b.cta]) {
            ...
            if (found)
                return reply.code(400).send({ error: `İzin verilmeyen içerik: "${found}"` });
        }
        ...
        if (b.mediaUrl) {
            try {
                ...
                return reply.code(400).send({ error: "Geçersiz medya URL" });
            }
        }
    ...
    app.get("/ads/campaigns/:id", async (req, reply) => {
        ...
        for (const imp of campaign.adImpressions) {
            ...
            if (!byDay[day])
                byDay[day] = { impressions: 0, clicks: 0 };
    ...
    app.post("/ads/track/:id", async (req) => {
        ...
        const { type = "impression", userId, postId } = req.body;
...
async function _setStatus(req, reply, status) {
    const userId = requireAuth(req, reply);
    if (!userId)
        return;
    const { id } = req.params;
    ...
    if (!c)
        return reply.code(404).send({ error: "Bulunamadı" });
    if (c.advertiserId !== userId)
        return reply.code(403).send({ error: "Yetkisiz" });
    if (c.status === "ended")
        return reply.code(400).send({ error: "Sonlanmış kampanya değiştirilemez" });
