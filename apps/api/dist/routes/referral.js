...
export async function registerReferralRoutes(app) {
    ...
    app.post("/referral/apply", async (req, reply) => {
        ...
        if (!referralCode?.trim())
            return reply.code(400).send({ error: "Referral kodu gerekli" });
        // Check if user already has a referrer
        const existing = await prisma.referralEntry.findUnique({ where: { referredId: userId } });
        if (existing)
            return reply.code(409).send({ error: "Zaten bir referral kodunuz var" });
    ...
    app.post("/referral/claim", async (req, reply) => {
        ...
        if (unpaid.length === 0)
            return reply.code(400).send({ error: "Bekleyen ödül yok" });
