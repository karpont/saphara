...
const POOLS = [
  {
    ...
    lockDays: 0, min: 100, max: 500_000,
    tvl: 842_000, stakers: 1_240,
  ...
  },
  {
    ...
    lockDays: 30, min: 500, max: 2_000_000,
    tvl: 2_340_000, stakers: 3_100,
  ...
  },
  {
    ...
    lockDays: 90, min: 1_000, max: 5_000_000,
    tvl: 5_120_000, stakers: 2_800,
  ...
  },
  {
    ...
    lockDays: 0, min: 50, max: 10_000_000,
    tvl: 1_890_000, stakers: 980,
...
const TIERS = [
  ...
  { name: "Gümüş",  minStake: 2_000,  multiplier: 4,  color: "#94a3b8", launchpadAlloc: true,  guaranteed: false },
  { name: "Altın",  minStake: 5_000,  multiplier: 10, color: "#f0b429", launchpadAlloc: true,  guaranteed: true  },
  { name: "Elmas",  minStake: 20_000, multiplier: 40, color: "#a5f3fc", launchpadAlloc: true,  guaranteed: true  },
];
...
function calcApy(aprDecimal: number): number {
  return (Math.pow(1 + aprDecimal / 365, 365) - 1) * 100;
}

function estimatedReward(amount: number, apyNum: number, days: number): number {
...
async function getNotifyList(): Promise<string[]> {
...
async function saveNotifyList(list: string[]) {
...
export async function registerStakingRoutes(app: FastifyInstance) {
  ...
  app.get("/staking/calculate", async (req) => {
    const { amount, poolId, days } = req.query as { amount?: string; poolId?: string; days?: string };
    const pool = POOLS.find(p => p.id === (poolId ?? "flexible"));
    if (!pool) return { error: "Geçersiz havuz" };
