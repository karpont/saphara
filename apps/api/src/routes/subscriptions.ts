...
export async function registerSubscriptionRoutes(app: FastifyInstance) {
  ...
  app.post("/subscriptions/subscribe", async (req, reply) => {
    const userId = requireAuth(req, reply);
    if (!userId) return;
    const { creatorId, pricePart, currency = "PART", txHash, durationDays = 30 } = req.body as any;
    if (!creatorId || !pricePart) return reply.code(400).send({ error: "creatorId ve pricePart gerekli" });
    if (creatorId === userId) return reply.code(400).send({ error: "Kendinize abone olamazsınız" });
