...
export async function registerNotificationSettingsRoutes(app: FastifyInstance) {
  ...
  app.post("/notifications/settings", async (req, reply) => {
    ...
    const { likes, comments, follows, tips, dm, reposts } =
      req.body as { likes?:boolean; comments?:boolean; follows?:boolean; tips?:boolean; dm?:boolean; reposts?:boolean };

    const data: Record<string, boolean> = {};
    if (likes    !== undefined) data.likes    = likes;
    if (comments !== undefined) data.comments = comments;
    if (follows  !== undefined) data.follows  = follows;
    if (tips     !== undefined) data.tips     = tips;
    if (dm       !== undefined) data.dm       = dm;
    if (reposts  !== undefined) data.reposts  = reposts;
