...
export async function registerAnalyticsRoutes(app: FastifyInstance) {
  app.get("/analytics/me", async (req, reply) => {
    ...
    const buckets: Record<string, { likes: number; comments: number; reposts: number; views: number }> = {};
    for (let i = 29; i >= 0; i--) {
    ...
    for (const p of posts) {
      ...
      if (buckets[key]) { buckets[key].likes += p.likes; buckets[key].comments += p.comments; buckets[key].reposts += p.repostCount ?? 0; }
    }
    for (const r of reels) {
      const key = r.createdAt.toISOString().slice(0, 10);
      if (buckets[key]) { buckets[key].likes += r.likes; buckets[key].views += r.views; }
    }
