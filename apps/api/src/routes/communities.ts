...
export async function registerCommunityRoutes(app: FastifyInstance) {
  ...
  app.get("/communities/:slug", async (req, reply) => {
    ...
    if (!community) return reply.code(404).send({ error: "Topluluk bulunamadı" });
    return community;
  });
  ...
  app.post("/communities", async (req, reply) => {
    ...
    if (!userId) return;
    const { name, description, imageUrl, private: isPrivate } = req.body as any;
    if (!name?.trim()) return reply.code(400).send({ error: "İsim gerekli" });
