...
export async function registerMarketRoutes(app: FastifyInstance) {
  ...
  app.get("/listings/:id", async (req, reply) => {
    ...
    if (!listing) return reply.code(404).send({ error: "İlan bulunamadı" });
  ...
  app.post("/listings", async (req, reply) => {
    ...
    const {
      title, description, imageUrl, pricePart,
      metadataURI, category = "digital", nftTokenId,
    } = req.body as any;
    ...
    if (nftTokenId) {
      ...
      if (!token)             return reply.code(404).send({ error: "NFT bulunamadı" });
  ...
  app.patch("/listings/:id", async (req, reply) => {
    ...
    if (!userId) return;
    const { id } = req.params as { id: string };
    const { pricePart, description, imageUrl } = req.body as any;

    const listing = await prisma.listing.findUnique({ where: { id } });
    if (!listing) return reply.code(404).send({ error: "İlan bulunamadı" });
    if (listing.sellerId !== userId) return reply.code(403).send({ error: "Bu ilan size ait değil" });
    if (listing.status !== "Listed") return reply.code(400).send({ error: "Satılan ilan güncellenemez" });
  ...
  app.delete("/listings/:id", async (req, reply) => {
    ...
    const listing = await prisma.listing.findUnique({ where: { id } });
    if (!listing) return reply.code(404).send({ error: "İlan bulunamadı" });
    if (listing.sellerId !== userId) return reply.code(403).send({ error: "Yetki yok" });
    if (listing.status !== "Listed") return reply.code(400).send({ error: "Aktif olmayan ilan silinemez" });
  ...
  app.post("/listings/:id/buy", async (req, reply) => {
    ...
    if (!listing)                  return reply.code(404).send({ error: "İlan bulunamadı" });
    if (listing.status !== "Listed") return reply.code(400).send({ error: "Bu ilan artık mevcut değil" });
    if (listing.sellerId === userId) return reply.code(400).send({ error: "Kendi ilanınızı satın alamazsınız" });
  ...
  app.post("/listings/:id/sync", async (req, reply) => {
    ...
    if (!existing) return reply.code(404).send({ error: "İlan bulunamadı" });
    if (existing.sellerId !== userId) return reply.code(403).send({ error: "Yetkisiz" });

    const { onchainId, status } = req.body as { onchainId?: string; status?: string };
