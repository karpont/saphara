...
async function verifyBscTx(txHash: string, currency: "PART" | "USDT" | "BNB"): Promise<{ ok: boolean; reason?: string }> {
  try {
    ...
    if (!receipt) return { ok: false, reason: "İşlem henüz onaylanmamış" };
    if (receipt.status !== "0x1") return { ok: false, reason: "İşlem başarısız (reverted)" };
    ...
    if (currency === "BNB") {
      ...
      if (!tx) return { ok: false, reason: "İşlem bulunamadı" };
      if (tx.to?.toLowerCase() !== TREASURY) return { ok: false, reason: "Alıcı treasury değil" };
      return { ok: true };
    }
    ...
    if (!transferLog) return { ok: false, reason: `${currency} treasury transferi logda bulunamadı` };

    return { ok: true };
  } catch (e) {
    return { ok: false, reason: `RPC hatası: ${(e as Error).message}` };
  }
}
...
export async function registerStoreRoutes(app: FastifyInstance) {
  ...
  app.get("/store", async (req) => {
    ...
    try {
      const items = await prisma.storeItem.findMany({
        where: { active: true, ...(kind ? { kind: kind as any } : {}) },
  ...
  app.post("/store/:itemId/buy", async (req, reply) => {
    ...
    const { txHash, currency = "PART" } = req.body as { txHash?: string; currency?: "PART" | "USDT" | "BNB" };

    const item = await prisma.storeItem.findUnique({ where: { id: itemId } });
    if (!item || !item.active) return reply.code(404).send({ error: "Ürün bulunamadı" });

    /* Ücretli ürünlerde txHash zorunlu ve BSC'de doğrulanır */
    const price = Number(item.pricePart ?? 0);
    if (price > 0) {
      if (!txHash) return reply.code(400).send({ error: "Ödeme txHash zorunlu" });
      const valid = await verifyBscTx(txHash, currency);
      if (!valid.ok) return reply.code(402).send({ error: `Ödeme doğrulanamadı: ${valid.reason}` });

      /* Aynı txHash daha önce kullanılmış mı? */
      const used = await prisma.userInventory.findFirst({ where: { txHash } });
      if (used) return reply.code(409).send({ error: "Bu işlem zaten kullanılmış" });
    }
  ...
  app.post("/inventory/:invId/equip", async (req, reply) => {
    ...
    if (!inv || inv.userId !== userId) return reply.code(403).send({ error: "Yetkisiz" });
  ...
  app.post("/store/admin/items", async (req, reply) => {
    ...
    const { name, imageUrl, pricePart, kind, active } = req.body as {
      name: string; description?: string; imageUrl?: string;
      pricePart?: number; kind?: string; rarity?: string; active?: boolean;
    };

    if (!name?.trim()) return reply.code(400).send({ error: "name zorunlu" });
