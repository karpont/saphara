...
export async function registerUploadRoutes(app: FastifyInstance) {
  ...
  app.post("/uploads/part", async (req, reply) => {
    const userId = requireAuth(req, reply);
    if (!userId) return;
    const { key, s3UploadId, partNumber } = req.body as { key: string; s3UploadId: string; partNumber: number };
    // Guvenlik: key kullanicinin kendi klasorunde mi?
    if (!key?.startsWith(`uploads/${userId}/`)) return reply.code(403).send({ error: "Yetkisiz anahtar" });
  ...
  app.post("/uploads/complete", async (req, reply) => {
    const userId = requireAuth(req, reply);
    if (!userId) return;
    const { key, s3UploadId, parts } = req.body as {
  ...
  app.post("/uploads/abort", async (req, reply) => {
    const userId = requireAuth(req, reply);
    if (!userId) return;
    const { key, s3UploadId } = req.body as { key: string; s3UploadId: string };
    if (!key?.startsWith(`uploads/${userId}/`)) return reply.code(403).send({ error: "Yetkisiz anahtar" });
