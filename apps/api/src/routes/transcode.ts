 ...
 */
export async function registerTranscodeRoutes(app: FastifyInstance) {
  // Karar: dosya boyutuna gore sunucu mu tarayici mi?
  app.get("/transcode/should-server", async (req) => {
    const sizeMb = Number((req.query as any).sizeMb ?? 0);
    return {
  ...
  app.post("/transcode", async (req, reply) => {
    ...
    if (!userId) return;
    const { inputPath, outputPath, preset } = req.body as any;
    if (!inputPath || !outputPath) return reply.code(400).send({ error: "inputPath ve outputPath gerekli" });
  ...
  app.get("/transcode/:id", async (req, reply) => {
    ...
    const job = getJob(id);
    if (!job) return reply.code(404).send({ error: "Is bulunamadi" });
