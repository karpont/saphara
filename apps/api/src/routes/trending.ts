 ...
 */
function velocityBoost(createdAt: Date): number {
...
function trendScore(post: {
  likes: number; comments: number; shares: number; repostCount: number;
  saveCount: number; views?: number; videoCompletion?: number;
  mediaType?: string | null; createdAt: Date;
  ...
  const mt = post.mediaType as string | null | undefined;
  if (mt === "video")       boost *= 1.5;
  else if (mt === "audio")  boost *= 1.2;
  else if (mt === "image")  boost *= 1.1;
  if (post.author?.verified) boost *= 1.1;
...
export async function registerTrendingRoutes(app: FastifyInstance) {
  ...
  app.get("/trending", async (req) => {
    const { limit = "10", window = "48" } = req.query as { limit?: string; window?: string };
    ...
    const [hashtags, rawPosts, trendingReels] = await Promise.all([
      ...
      prisma.post.findMany({
        where: { createdAt: { gte: new Date(Date.now() - windowHrs * 3_600_000) } },
        take: 200,
        include: {
...
export async function upsertHashtags(text: string, postId: string) {
