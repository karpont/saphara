...
let lastReport: Awaited<ReturnType<typeof generateReport>> | null = null;
...
async function scan() {
  try {
    ...
    log.error("bot tarama hatasi", { err: (e as Error).message });
  }
}
...
async function adSweep() {
  try {
    ...
    if (isDbOffline(e)) return;
    log.error("reklam sweep hatasi", { err: (e as Error).message });
  }
}

function notifyAdvertiser(userId: string, text: string) {
  try {
